...
2. **Add persistent storage.** Provision Neon Postgres, then implement the existing `benches`, `adopters`, and `adoptions` schema there. Keep the same public name, inclusive date range, and integer-cent amount rules. Keep local SQLite as a development option while Vercel uses a `DATABASE_URL`-configured Neon connection. Use a transaction that locks the selected bench row before checking and inserting a booking; preserve the current guarantee that only one unexpired booking can reserve a bench. Use Neon's pooled connection string for Vercel Functions. See [Postgres on Vercel](https://vercel.com/docs/postgres) and [Neon pooling](https://neon.com/docs/connect/connection-pooling).
3. **Move initialization out of hosted app startup.** Keep local SQLite initialization, but use a repeatable Postgres schema migration and a separate, idempotent seed command for the hosted app. Run them once against the Neon database, not on every function start. Start with fresh fictional data.
...
- Preview and Production share one persistent Neon database; neither writes to a local SQLite file.
